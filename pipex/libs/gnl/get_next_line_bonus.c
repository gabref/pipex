...
/*   Updated: 2024/02/07 17:28:04 by galves-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_fd_add_cache(int fd, char **cache)
{
	...
	while (rbytes > 0 && !has_newline(last_cache_len, *cache))
	{
		...
		if (rbytes == -1)
			return (0);
		buf[rbytes] = '\0';
		last_cache_len = ft_strlen_g(*cache);
		*cache = strjoin_g(*cache, buf, last_cache_len, rbytes);
		if (*cache == NULL)
			return (0);
	}
	free(buf);
...
char	*get_next_line(int fd)
{
	...
	if (!read_fd_add_cache(fd, &cache[fd]))
		return (NULL);
	cache_len = ft_strlen_g(cache[fd]);
